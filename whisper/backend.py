...
# 从服务器端获取音频转文字结果
def voice_to_text_server(audio_data, sender_ws, loop):
    resp = requests.post(
        server,
        json={"audio": audio_data.tolist()}
    )

    original_text = resp.json()["origin"]
    translated_text = translate.google_web_translate(resp.json()["translated"])
...
# 翻译音频并输出中文
def get_audio_text(audio_data, sender_ws, loop):
    
    voice_to_text_server(audio_data, sender_ws, loop)
